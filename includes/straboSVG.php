	...
	class straboSVG{
		...
		function straboSVG($allspots,$strat_section){
			...
			if($this->isOldVersion($allspots)){
				//echo "This strat section was created with an old version of the Strabo App and is no longer compatible with strat section export.";
				
				//$this->dumpVar($_SERVER['REQUEST_URI']);
				...
				//$this->dumpVar($centroid);
 				...
 				if($spot['geometry']->geometries != ""){
 					...
 					foreach($spot['geometry']->geometries as $geo){
						...
						//$this->dumpVar($individualCentroid);
		...
		function getClasses($spot, $isInterbed){
			...
			//echo "getclassesisinterbed: $isInterbed<br>";
			...
			//$this->dumpVar($spot); $this->dumpVar($patterndir);
		...
		function getPatternDir($spot, $isInterbed){
				...
				//$this->dumpVar($isInterbed);
				$spot = (object) $spot;
				$featureProperties = (object) $spot->properties;
				//$this->dumpVar($featureProperties);exit();
				...
				//if($spot->properties[id] == "15928527859896"){
					...
					if ($featureProperties->sed && $featureProperties->sed->lithologies && $featureProperties->strat_section_id) {
						...
						//$this->dumpVar($stratSectionSettings);exit();
						/*
						...
						*/
						...
						if ($stratSectionSettings && $stratSectionSettings->column_profile === 'basic_lithologies') {
							...
							//echo "primary lithology: $lithology->primary_lithology <br>";
							...
							if ($lithology->primary_lithology == 'siliciclastic'){
								...
								//echo "siliciclastic_type: $siliciclastic_type<br>";
						...
						if($returnpair == "" && $stratSectionSettings){
							...
							/*
							if( $lithologies->mud_silt_grain_size!="" ){
								$grainSize = $lithologies->mud_silt_grain_size;
							}elseif( $lithologies->sand_grain_size!="" ){
								$grainSize = $lithologies->sand_grain_size;
							}elseif( $lithologies->congl_grain_size!="" ){
								$grainSize = $lithologies->congl_grain_size;
							}elseif( $lithologies->breccia_grain_size!="" ){
								$grainSize = $lithologies->breccia_grain_size;
							}elseif( $lithologies->principal_dunham_class!="" ){
								$grainSize = $lithologies->principal_dunham_class;
							}elseif( $lithologies->primary_lithology!="" ){
								$grainSize = $lithologies->primary_lithology;
							}
							*/
				...
				//$this->dumpVar($spot->properties[id]); //exit();
