...
*/
	...
	function oldgetIconForFeature20240613(feature) {
	let feature_type = 'none';
	let rotation = 0;
	let symbol_orientation = 0;
	let orientation_type = 'none';
...
//add features from a single dataset
	...
	//$.post( "interfacesearch.php?dsids=3-"+id, querystring, function( result ) {
	//https://strabospot.org/doi/doisearch.php?u=0021a279-6e8d-4176-a9f5-94f1742fe128-17158683183764
	$.post( "https://strabospot.org/doi/doisearch.php?u="+projectuuid+"-"+id, function( result ) {
	...
	//$.getJSON('newsearchspots.json?env='+env+'&dsets='+id,function(result){
		...
		_.each(result.datasets, function(newds){
			//Strip UUID from beginning of id

			////console.log("Strip here...");
			////console.log(newds);

			//const last6 = str.slice(-6);
			//let fixedid = newds.id.slice(-14);
...
//fetch features from database
	...
	if((loadedFeatures=="" || envelopeOutside()) && !currentlyLoading){
		...
		$.getJSON('snewsearchspots.json?env='+env,function(result){
...
//get info for current spot and tab and update UI
	...
	////console.log("sidebar name: " + activeTabName);
...
function stratStyleFunction(feature, resolution) {
	...
	newurl = "ssnewsearchdatasets.json?"+newurl;
...
const delay = ms => new Promise(res => setTimeout(res, ms));
	...
	let loadDOI = async (uuid) => {

	console.log(sample_vars);

	$('#spotswaiting').show();
	await delay(1000);

	$.post( "https://strabospot.org/doi/doiproject.php?u="+uuid, function( result ) {
		console.log(result);

	let datasets = result.datasets;
		projectuuid = result.uuid;
		projectdoi = result.doi;


		$('#projectTitle').html('Project Name: ' + result.projectname);

		if(result.doi != null){
			$('#projectDOI').html('DOI: ' + result.doi);
			$('#projectDOI').show();
		}

		$('#projectDate').html('Published: ' + result.datecreated);
		$('#projectOwner').html('Owner: ' + result.owner);


		$('#projectBox').show();

		//Update interface here (project name, date, etc...)
	...
	let currentZoom = map.getView().getZoom();
			if(currentZoom < 12){
				panToCoords(longitude, latitude, 12);
			}else{
				panToCoords(longitude, latitude, currentZoom);
			}
...
//var gotoDataset = function(datasetid){
	...
	let oldgotoDataset20240516 = async (datasetid) => {
//var gotoDataset = function(datasetid){
	$('#spotswaiting').show();
	await delay(3000);
	...
	let expandDataset = function(dataset){
	////console.log("expanding");
	expandedDatasets.push(dataset);
	mapDataset(dataset.id);
	updateShownDatasets();
}

	let mapDataset = function(id){
	addFeatures(id);
}
	...
	let thishtml = '';
		_.each(expandedDatasets, function(ds){
			...
			//thishtml += '<div>'+projectname + ' - ' + name+' ('+count+' spots) (Owned by '+owner+')</div>';
			thishtml += '<div>'+name+' ('+count+' spots) (Owned by '+owner+')</div>';
		});
...
function getDatasetIdForSpotId(spotId){
	...
	let getvars = buildGetSearchVars();
		...
		//new
		//window.open('doisearchdownload?type='+datatype+'&dsids='+dsids+'&range='+range+'&envelope='+envelope+'&querystring='+querystring);

		console.log('doisearchdownload?type='+datatype+'&u='+projectuuid);
		window.open('../doisearchdownload?type='+datatype+'&u='+projectuuid);
