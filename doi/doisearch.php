...
/**
 * File: doisearch.php
 * Description: Manages Digital Object Identifier (DOI) operations
 *
...
//https://strabospot.org/doi/doisearch.php?u=0021a279-6e8d-4176-a9f5-94f1742fe128&d=17158683183764
//https://strabospot.org/doi/doisearch.php?u=0021a279-6e8d-4176-a9f5-94f1742fe128-17158683183764

$u = $_GET['u'] ?? '';
$u = preg_replace('/[^a-zA-Z0-9\-]/', '', $u);
if($u == "") die("No id provided.");

//Break apart u to get uuid and dataset id
$pos = strrpos($u, "-");

$uuid = substr($u, 0, $pos);

//Todo: gather owner for placement in file
$owner = "John Smith";

$datasetid = substr($u, $pos + 1);
...
foreach($datasets as $key=>$value){
	if($key == $datasetid) $dataset = $value;
}

if($dataset == "") exit("Dataset not found.");
...
$outDataset = new stdClass();
$outDataset->name = $dataset->name;
$outDataset->id = $u;
$outDataset->spotcount = count($dataset->spotIds);

$out->datasets[] = $outDataset;
...
foreach($json->spotsDb as $key=>$spot){
	if(in_array($key, $lookSpots)){
		...
		if($spot->properties->image_basemap){
			$imagebasemaps[] = $spot->properties->image_basemap;
			$hasimagebasemap = "yes";
		}
