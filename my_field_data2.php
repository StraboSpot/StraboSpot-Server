...
<?php
	foreach($collaboration_rows as $crow){
		...
		$owner_row = $db->get_row("select * from users where pkey = $collab->project_owner_user_pkey");
		$owner_name = $owner_row->firstname." ".$owner_row->lastname;
		...
		$collabcount = $db->get_var("select count(*) from collaborators where strabo_project_id = '$projectid' and project_owner_user_pkey = $userpkey and accepted = true");
