	...
	let basicLitholigiesLabels = ['other', 'coal', 'mudstone', 'sandstone', 'conglomerate/breccia', 'limestone/dolostone'];
	//var grainSizeOptions = DataModelsFactory.getSedLabelsDictionary();
	let spotsWithStratSections = {};
	let stratSectionSpots = {};
	let xInterval = 10;  // Horizontal spacing between grain sizes/weathering tick marks
	let yMultiplier = 20;  // 1 m interval thickness = 20 pixels
	let grainSizeOptions = {
							  "clastic": [
								{
								  "value": "clay",
								  "label": "clay"
	...
	let intervalSpots = getStratIntervalSpots();
	let sectionHeight = 0;

	let fc = turf.featureCollection(intervalSpots);
	let envelope = turf.envelope(fc);
	...
	let zoom = map.getView().getZoom();
		...
		// Y Axis
	let currentSectionHeight = getSectionHeight();
	let yAxisHeight = currentSectionHeight + 40;

		ctx.beginPath();
		ctx.setLineDash([]);
	let p = getPixel([0, 0], pixelRatio);
		ctx.moveTo(p.x, p.y);
	...
	let fc = turf.featureCollection([intervalSpot]);
	let envelope = turf.envelope(fc);
	let y = envelope.geometry.coordinates[0][3][1];

	let label = roundToDecimalPlaces(y / yMultiplier, 2);
...
// 		}
	...
	let p = y === 0 ? getPixel([-10, y], pixelRatio) : getPixel([0, y], pixelRatio);
		ctx.beginPath();
		ctx.moveTo(p.x, p.y);
	let xAxisLength = (_.size(labels) + spacing) * xInterval;
	...
	let lithologies = featureProperties.sed.lithologies;
	let lithologyField = lithologies[n].primary_lithology;
	let grainSize = getGrainSize(lithologies[n]);
	let stratSectionSettings = currentStratSection.properties.sed.strat_section;
	...
	let extent = featureProperties.geometry.getExtent();
	let width = 10 / resolution * 2;
	let height = (extent[3] - extent[1]) / resolution * 2;
					canvas.width = width;
