<?php
/**
 * File: ApparatusController.php
 * Description: ApparatusController class
 *
...
class ApparatusController extends MyController
{
	public function getAction($request) {

		if(isset($request->url_elements[2])) {
			$apparatus_id = $request->url_elements[2];
			...
			if(isset($request->url_elements[3])) {
				...
				$data = $this->expclass->getApparatus($apparatus_id);

				if($data->Error!=""){
					header("Apparatus not Found", true, 404);
				}

			}
		} else {
			header("Bad Request", true, 400);
			$data["Error"] = "Apparatus UUID must be provided";
		}
		return $data;
	}
	...
	public function postAction($request) {
		...
		if($apparatusname==""){
			...
			$data = $this->expclass->insertApparatus($injson, $uuid);
