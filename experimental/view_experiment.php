<?php
/**
 * File: view_experiment.php
 * Description: Displays experiment details, parameters, and results
 *
...
//$row = $db->get_row("select * from straboexp.experiment where pkey = $experiment_pkey and userpkey = $userpkey");


//check once to see if it is public or owned
$count = $db->get_var_prepared("
	SELECT count(*) FROM straboexp.project p,
							straboexp.experiment e
						WHERE
							p.pkey = e.project_pkey
							AND (p.ispublic OR p.userpkey = $1)
							AND e.pkey = $2
", array($userpkey, $experiment_pkey));

if($count == 0) die("Experiment not found.");

$row = $db->get_row_prepared("SELECT * FROM straboexp.experiment WHERE pkey = $1", array($experiment_pkey));
...
<div class="topTitle">Experiment: <?php echo $row->id?></div>
...
/*
<?php echo $ch->number?> - <?php echo $ch->header->type?>
...
document.addEventListener("DOMContentLoaded", function(event){
	exper_fetchExperimentForViewOnly(<?php echo $experiment_pkey?>);
	exper_fixButtonsAndDivs();
