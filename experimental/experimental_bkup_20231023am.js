...
function doSubmitNewProject() {
	let error = checkForProjectSubmitErrors();
	if(error != ""){
		alert(error);
	}else{
		//OK, now create JSON for POSTING to the server
	let outJSONString = buildProjectJSON();
		...
		$.post( "inNewProject.php", {json: outJSONString})
			.done(function( data ) {
				if(data.Error){
					...
					<div style="text-align:center;padding-top:25px;"><button class="submitButton" style="vertical-align:middle" onclick="window.location.href = \'/my_data#straboexperimental\';"><span>Continue </span></button></div>\
					');
...
function buildProjectJSON() {
	...
	let outJSONString = JSON.stringify(outObject);
...
function checkForApparatusSubmitErrors() {
	...
	let docRows = documentsTable.children;
	if(docRows.length > 0){
		for(let i = 0; i < docRows.length; i++){
	let rowNum = i + 1;
	let thisFile = docRows[i].children[0].children[0].children[2].children[0].children[1].value;
...
function doSubmitNewApparatus() {
	let error = checkForApparatusSubmitErrors();
	if(error != ""){
		alert(error);
	}else{
	...
	let facilityPkey = document.getElementById("facilityPkey").value;
		fd.append('facility_pkey', facilityPkey);

		//Build JSON
	let jsonString = buildApparatusJSON();
		...
		if(docRows.length > 0){
			for(let i = 0; i < docRows.length; i++){
	let fileNode = docRows[i].children[0].children[0].children[2].children[0].children[1];
				if(fileNode.type == "file"){
	let uuid = docRows[i].children[0].children[0].children[3].children[0].children[2].value;
...
function doSubmitEditApparatus() {
	...
	let fd = new FormData();
		...
		//Gather Facility Pkey
	let apparatusPkey = document.getElementById("apparatusPkey").value;
...
function buildApparatusJSON() {
	...
	let fullPath = docRows[i].children[0].children[0].children[2].children[0].children[1].value;
	let fileName = fullPath.split(/[\\\/]/).pop();
				//alert(fileName);
			}else{
				//get file name from div
	let fileName = docRows[i].children[0].children[0].children[2].children[0].children[1].children[0].innerHTML;
			}
...
function doSubmitNewFacility() {
	let error = checkForFacilitySubmitErrors();
	if(error != ""){
		alert(error);
	}else{
		//OK, now create JSON for POSTING to the server
	let outJSONString = buildFacilityJSON();
...
function doMultiFileDebug() {
	...
	let file1files = $('#foofile1')[0].files;
	fd.append('foofoofile1id',file1files[0]);

	let file2files = $('#foofile2')[0].files;
...
function addApparatusDocument() {
	...
	let newRowNum = existingRows.length + 1;
...
function deleteApparatusDocument(rowNum){
	...
	for(let i = 0; i < documentsTable.children.length; i++){
	let locNum = i + 1;
...
function addApparatusParamRow(){
	...
	//Figure out what row we're on
	let newRowNum = paramsTable.children.length;
	...
	let myChildren = newParam.children;
...
function moveApparatusParamUp(rowNum){
	...
	let toPos = rowNum - 1;
	let numRows = paramsTable.children[0].children.length - 1;
...
// ************************************************************************************************************************************************************
	...
	let tempFacilityData = null;
	let tempApparatusData = null;
	let tempDaqData = null;
	let tempSampleData = null;
	let tempExperimentData = null;
	let tempDataData = null;
			...
			for(let deviceNum = 0; deviceNum < daqData.devices.length; deviceNum ++){
				...
				if(device.channels != null){
					...
					for(let channelNum = 0; channelNum < daqData.devices[deviceNum].channels.length; channelNum ++){
						...
						let headerVal = channel.header.type;
						let channelNumVal = channel.number;
						if(channelNumVal != ""){
							newButton.children[0].innerHTML = channelNumVal + " - " + headerVal;
						}else{
							newButton.children[0].innerHTML = headerVal;
						}
...
function doSubmitNewExperiment() {
	let error = exper_CheckForExperimentSubmitErrors();
	if(error != ""){
		alert(error);
	}else{
	...
	let fd = new FormData();
		...
		//Gather Facility Pkey
	let projectPkey = document.getElementById("projectPkey").value;
			...
			success:function(data){
				if(data.Error){
					...
					setTimeout(function () {
						...
						<div style="text-align:center;padding-top:25px;"><button class="submitButton" style="vertical-align:middle" onclick="window.location.href = \'/my_data\';"><span>Continue </span></button></div>\
						');
...
function doSubmitEditExperiment() {
	...
	let fd = new FormData();
		...
		//Gather Facility Pkey
	let experimentPkey = document.getElementById("experimentPkey").value;
...
function exper_fillApparatusList(data) {
	...
	for(let fnum = 0; fnum < facilities.length; fnum++){
	let f = facilities[fnum];
		...
		for(let anum = 0; anum < f.apparatuses.length; anum++){
	let a = f.apparatuses[anum];
			html += '<tr>';
...
function exper_doSaveFacilityApparatusInfo() {
	let error = exper_checkForFacilityApparatusSubmitErrors();
...
function exper_doSaveDAQInfo() {
	let error = exper_checkForDAQSubmitErrors();
	...
	*/
		...
		for(let d = 0; d < deviceRows.length; d ++){
			...
			if(channelRows != null){
				...
				for(let ch = 0; ch < channelRows.length; ch ++){
					...
					channelHeaderSelect.setAttribute('onchange','exper_daqRenameChannelButton(' + d + ', ' + ch +');');
			...
			let documentButtonRows = deviceRows[d].children[0].children[1].children[0].children[0].children[1].children[1].children[0].children[0].children[0];
			...
			let documentRows = deviceRows[d].children[0].children[1].children[0].children[0].children[1].children[1].children[0].children[1].children[0];
			//let documentRows = document.getElementById("daq_device_documents_" + d);
	...
	if(sampleParameterRows != null){
		...
		for(rowNum = 0; rowNum < rowCount; rowNum++){
			...
			parameterVariableSelect.setAttribute('onchange','exper_sampleRenameParameterButton(' + rowNum +');');
	...
	if(sampleDocumentRows != null){
		...
		for(rowNum = 0; rowNum < rowCount; rowNum++){
			...
			let documentTypeSelect = findSubNode(sampleDocumentRows[rowNum], "docType");
			...
			documentTypeSelect.setAttribute('onchange','exper_sampleRenameDocumentButton(' + rowNum +');');
	...
	if(experimentDocumentRows != null){
		...
		for(rowNum = 0; rowNum < rowCount; rowNum++){
			...
			let documentTypeSelect = findSubNode(experimentDocumentRows[rowNum], "docType");

			documentTypeSelect.setAttribute('onchange','exper_experimentRenameDocumentButton(' + rowNum +');');
...
function exper_addDAQDevice() {
	...
	//Figure out what row we're on
	let newRowNum = existingRows.length;
...
function exper_daqRenameChannelButton(deviceRowNum, channelRowNum){
	...
	exper_updateDAQHeaderInputs(deviceRowNum, channelRowNum);
}
...
function exper_updateDAQHeaderInputs_with_div(headerRow, headerButton){
	...
	if(spec_a_vals.length > 0){
		//Create select
	let spec_a_select = document.createElement("select");
	...
	let spec_a_input = document.createElement("input");
	...
	if(spec_b_vals.length > 0){
		//Create select
	let spec_b_select = document.createElement("select");
	...
	let spec_b_input = document.createElement("input");
...
function exper_doSaveSampleInfo() {
	let error = exper_checkForSampleSubmitErrors();
...
function exper_buildExperimentData(){
	...
	let thisDoc = new Object();
		...
		if(findSubNode(docs[x], "docFile").type == "file"){
			//get file name from input - remove path first
	let fullPath = findSubNode(docs[x], "docFile").value;
	let fileName = fullPath.split(/[\\\/]/).pop();
		}else{
			//get file name from div
	let fileName = findSubNode(docs[x], "docFile").innerHTML;
		}
...
function exper_doSaveDataInfo() {
	let error = exper_checkForDataSubmitErrors();
	...
	if(datasetRows != null){
		...
		for(let dNum = 0; dNum < datasetRows.length; dNum++){
			...
			if(dataFile != null){
				if(dataFile.type == "file"){
					//get file name from input - remove path first
	let fullPath = dataFile.value;
	let fileName = fullPath.split(/[\\\/]/).pop();
				}
			}else{
	let fileName = findSubNode(dRow, "filename").innerHTML;
			}
		...
		for(let dNum=0; dNum < dataData.datasets.length; dNum++){
			...
			dataString += '					<div>\
												<div class="frontBoxRow">\
													...
													<div class="dataLeft w33">' + dRow.type + '</div>\
												</div>\
											</div>';
		}
...
function fadeOutCopiedMessage() {
	...
	let loader = document.getElementById("dataCopiedModal");
	...
	/*
	let fadeTarget = document.getElementById("dataCopiedModal");
